...
void TileEditor::save_map_script() {
    ...
    if (f.is_open()) {
        f << "# Map script (auto-generated by editor)\n";
        f << "# Modify this file to customize map behavior.\n";
        f << "# Tile data is in the companion .json file.\n\n";
        f << "proc map_init():\n";
...
void TileEditor::load_map_script(const std::string& map_path) {
    ...
    // Try to load existing script body (extract lines inside map_init)
    map_script_body_.clear();
    auto data = eb::FileIO::read_file(map_script_path_);
    if (!data.empty()) {
        std::string src(data.begin(), data.end());
        // Find "proc map_init():" and extract indented lines after it
        auto pos = src.find("proc map_init():");
        if (pos != std::string::npos) {
            pos = src.find('\n', pos);
            if (pos != std::string::npos) {
                pos++; // skip the newline
                // Collect all lines that start with whitespace (indented = body)
                while (pos < src.size()) {
                    auto eol = src.find('\n', pos);
                    ...
                    if (!line.empty() && (line[0] == ' ' || line[0] == '\t')) {
                        ...
                        break; // Non-indented line = next proc
                    }
                    pos = eol + 1;
                }
            }
        ...
        if (se) {
            ...
            if (ImGui::BeginChild("ScriptList", ImVec2(180, 0), true)) {
                ...
                if (!map_script_path_.empty()) {
                    ...
                    if (ImGui::Selectable(map_label.c_str(), msel)) {
                        // Load map script into the editor buffer
                        map_script_selected = true;
                        selected_script = -1;
                        // Build full file content from body
                        std::string full = "# Map script (auto-generated by editor)\n\nproc map_init():\n";
                        full += map_script_body_.empty() ? "    log(\"Map loaded\")\n" : map_script_body_;
                        int len = std::min((int)full.size(), (int)sizeof(script_buffer) - 1);
                        std::memcpy(script_buffer, full.data(), len);
                        script_buffer[len] = '\0';
